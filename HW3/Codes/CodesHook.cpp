...
void setHook()
{
	...
	len_override = 8; // nubmer of bytes to override, changes from function to function

	strcmp_continue_func = (LPVOID)((char *)f + len_override); // strcmp start with 2 cmd of 4 bytes each, so if we override first 5 we must add 3 when returning
